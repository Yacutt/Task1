        ...
        while (sumHealth(playersUnit) || sumHealth(botsUnit)) {
            if (!sumHealth(playersUnit)) {
                ...
                if (in.nextInt() == 0) {
                    ...
                    playersUnit.get(indexOf(playersUnit, hero)).attack(botsUnit.get(indexOf(botsUnit,attackedHero)),
                            botsUnit, battlefield);
                    printAllUnit(playersUnit, botsUnit);
                } else if (in.nextInt() == 1) {
                    // todo
                    playersUnit.get(indexOf(playersUnit, hero))
                            .movePlayer(unitDefinitionReverse(playersUnit.get(indexOf(playersUnit, hero))),
                                    playersUnit, battlefield);
                    battlefield.draw();
                }
